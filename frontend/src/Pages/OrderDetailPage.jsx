...
import { ArrowLeftIcon, HeadphonesIcon, Layout, LayoutListIcon, LockIcon, MessageCircle, MessageCircleIcon } from 'lucide-react'
...
function OrderDetailPage() {
  ...
  if ( error || !order ) {
    return (
      <PageError message="Order not found." action={{ to: "/order", label: "Back to order"}} />
    )
  }
  ...
  return (
    <div className='space-y-8 text-left'>
      <Link to="/orders" className='btn btn-ghost btn-sm gap-2 px-0 text-base-content/70 hover:text-prim'>
      <ArrowLeftIcon className='size-4' aria-hidden /> Back to orders
      </Link>
      ...
      <div className='overflow-hidden rounded-2xl border border-base-300 bg-base-100 shadow-lg'>
        ...
        <span className={`badge badge-lg w-fit capitalize lg:ml-auto ${
          paid
          ? "badge-success"
        ...
        Need hel with shipping or return? Open the{" "}
        <strong className='text-base-content'>
          Support chat
        </strong>
        tab after payment. Video call links ae shared in that thread; everyone joins with the same link.
        </p>
        </div>
      </div>
