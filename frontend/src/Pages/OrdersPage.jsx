        ...
        return (
            <div className="text-left">
                ...
                <p className="mb-8 text-sm text-base-content/70">
                    {staff ? "All store orders. Open one for customer support chat." : "Paid orders include customer support: open an order for chat."}
                </p>
                ...
                {orders.length === 0 ? (
                    ...
                    <ul className="space-y-4">
                        {orders.map(o => {
                            ...
                            const summary =
                            lineCount === 0 ? "No line items" : lineCount === 1 ? `${totalUnits} ${totalUnits === 1 ? "items" : "items"}`
                            : `${lineCount} products · ${totalUnits} items`

                            return(
                                <li key={o.id}>
                                    <Link to={`/orders/${o.id}`} className="group card border border-base-300 bg-base-100 shadow-sm transition hover:border-primary/45 hover:shadow-md">
                                    <div className="card-body flex-row flex-wrap items-center gap-4 py-5 sm:gap-5">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <span className={`badge badge-sm capitalize ${
                                                    o.status === "paid" ? "badge-success" : o.status === "pending" ? "badge-warning" : "badge-error"
                                                }`}>
                                                    {o.status}
                                                </span>
                                            </div>
                                        ...
                                        <div className="flex shrink-0 items-center gap-3">
                                            <div className="text-right">
                                                ...
                                                <p className="text-lg font-bold tabular-nums text-base-content sm:text-x1">
